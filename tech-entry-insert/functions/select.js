exports.index = async (event, rds, squel, rds_connect, formatter) => {
  ...
  if(query_list.includes("agent")){
    const agent_select = squel.select()
                              .from("agents")
    ...
    const agent_list = rds_connect.data_api("agent_list", agent_select, rds);
    result_list.push(agent_list);
  }

  if(query_list.includes("channel")){
    const channel_select = squel.select()
                                .from("channels")
                                .field("id")
                                .field("channel")
                                .order("id")
    ...
    const channel_list = rds_connect.data_api("channel_list", channel_select, rds);
    result_list.push(channel_list);
  }

  if(query_list.includes("industry")){
    const industry_select = squel.select()
                                .from("sub_industries")
                                .join("main_industries", null, "sub_industries.main_industry_id = main_industries.id")
    ...
    const industry_list = rds_connect.data_api("industry_list", industry_select, rds);
    result_list.push(industry_list);
  }

  if(query_list.includes("job")){
    const job_select = squel.select()
                            .from("sub_jobs")
    ...
    const job_list = rds_connect.data_api("job_list", job_select, rds);
    result_list.push(job_list);
  }

  if(query_list.includes("client_tech_industry")){
    const client_tech_industry_select = squel.select()
    ...
    const client_tech_industry_list = rds_connect.data_api("client_tech_industry_list", client_tech_industry_select, rds);
    result_list.push(client_tech_industry_list);
  }

  if(query_list.includes("address")){
    const address_select = squel.select()
                                .from("addresses")
                                .field("id")
                                .field("address")
                                .order("id")
    ...
    const address_list = rds_connect.data_api("address_list", address_select, rds);
